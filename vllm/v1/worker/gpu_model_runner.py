...
class GPUModelRunner(
    ...
    def _build_attention_metadata(
        ...
        # Experimental BFLA accuracy run: allow sparse prefill even when
        # prefix caching is enabled, but only for pure prefill-ish requests.
        bfla_allow_sparse_prefill = all(
            (self.requests[req_id].sampling_params is not None
             and self.requests[req_id].sampling_params.max_tokens <= 4096)
        ...
        cm_base = CommonAttentionMetadata(
            ...
            bfla_allow_sparse_prefill=bfla_allow_sparse_prefill,
        )
